...
/// Header prepended to every emitted capture.
///
/// `rate:` is recomputed from the CSV's timestamp column on load, so the value
/// written here never reaches the scheduler. It is emitted anyway because
/// `normalize` requires the field — a capture without it does not compile —
/// which is exactly why it needs saying: an editable number that silently does
/// nothing is worse than no number.
fn emitted_header() -> String {
    format!(
        "\
# Generated by `sonda new --from-prometheus`. Replays the capture verbatim.
#
# `rate:` below is informational. It is recomputed on load from the CSV's
# timestamp column ({}), so editing it has no effect.
# The field is required by the compiler, which is why it is written at all.
# To change replay speed, set `timescale:` instead.
",
        crate::config::RATE_FORMULA_PROSE
    )
}
...
pub fn to_yaml(file: &ScenarioFile) -> Result<String, SondaError> {
    ...
    let body = serde_yaml_ng::to_string(&value).map_err(|e| {
        SondaError::Config(ConfigError::invalid(format!(
            "acquire: emitted scenario could not be rendered: {e}"
        )))
    })?;
    Ok(format!("{}{body}", emitted_header()))
}
...
mod tests {
    ...
    #[test]
    fn emitted_header_states_that_rate_is_derived() {
        let grid = Grid::new(0.0, 2.0, 1.0).expect("grid");
        let series = [norm("m", &[], &[Some(1.0), Some(2.0), Some(3.0)])];
        let file = scenario_for("capture.csv", grid, &series, 1.0).expect("scenario");
        let yaml = to_yaml(&file).expect("yaml");
        ...
        assert!(
            yaml.starts_with("# Generated by"),
            "the header must lead the file, got: {}",
            &yaml[..yaml.len().min(80)]
        );
        assert!(
            yaml.contains("`rate:` below is informational"),
            "the header must say rate is derived"
        );

        // The claim has to stay true: the field is still written, because the
        // compiler requires it.
        assert!(
            file.scenarios.iter().all(|e| e.rate.is_some()),
            "rate is required by normalize; emitting none would not compile"
        );

        // And a header comment must not stop the file parsing.
        crate::compiler::parse::parse(&yaml).expect("emitted capture must still parse");
    }
