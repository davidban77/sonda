...
!!! tip "Want portable scenarios instead of raw replay?"
    ...
    original file, use [`sonda new --from`](from-csv.md) instead.

!!! tip "Replaying logs instead of metrics?"
    The same workflow works for log events with `log_csv_replay`. Export the window from Loki via `logcli`, run it through `jq` to produce a `timestamp,severity,message,...fields` CSV, and point a logs scenario at the file. The rate-derivation, `timescale`, and override-warn semantics described above apply identically. Walkthrough: [Log CSV Replay](log-files.md).
