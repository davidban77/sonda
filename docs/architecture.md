...
- **Deviation / DeviationReplace** — one change an extension makes to a base metric: `replace:` (field-wholesale) or `not_supported:` (removes), exactly one per deviation, at most one deviation per selector.
- **packs::extend::materialize()** — the pure pre-pass, `(extension, base) -> MetricPackDef`. Folds one `extends:` link: base metrics, then deviations applied, then the extension's additive metrics, with the extension's `shared_labels` merged over the base's. `compiler::expand::resolve_pack_chain` walks the chain through the resolver and folds with this, re-validating addressability after each step. The result is indistinguishable from a hand-written pack, so no later phase knows extension exists — and the materializer compiles into `sonda-wasm`, so the playground can show a pack's effective contents.
