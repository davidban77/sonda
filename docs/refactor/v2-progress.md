...
| **9b — v1 loader removal + server v2 (full retirement, scope expanded 2026-04-18)** | Delete `MultiScenarioConfig` type + its `pub use` from `sonda-core/src/lib.rs`. Delete `is_flat_single_scenario`, `load_flat_single_scenario`, `load_multi_config` + its tests, v1 parse branches in `load_config`/`load_log_config`/`load_histogram_config`/`load_summary_config`/`parse_builtin_scenario`. Refactor `run_multi` to take `Vec<ScenarioEntry>` directly (no wrapper). Update `sonda-server/src/routes/scenarios.rs` to accept v2 YAML/JSON bodies via `compile_scenario_file` — closes rows 13.1–13.9 (server API v2). Rewrite internal multi_runner + server tests that currently construct `MultiScenarioConfig`. **Full v1 retirement. Breaking public API change + server contract change.** Originally staged separately as 9d; bundled into 9b after discovering `MultiScenarioConfig` is also a runtime/server data carrier, making a clean-break scope indivisible. | architectural | Opus reviewer direct + UAT + doc |
...
**9d (Server API v2)** — **no longer a separate PR.** Bundled into 9b on 2026-04-18 because `MultiScenarioConfig` deletion + server v2 acceptance are tightly coupled: you can't delete the wrapper type without also updating the server endpoint that accepts it, and keeping the wrapper alive violates full v1 retirement. 9b completes both in one atomic PR.
